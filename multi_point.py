...
# 初始化GPS和AHRS的資料結構
data_list = []  # 用於存儲收集到的所有資料
gps_data = {"Heading": None, "Lat": None, "Lon": None}  # 用於存儲GPS資料
attitude_data = {"Roll": None, "Pitch": None, "Yaw": None}  # 用於存儲AHRS資料
last_update_time = {"gps": 0, "ahrs": 0}  # 記錄上次更新資料的時間
# 生成時間戳並創建文件名稱
file_timestamp = datetime.strftime(
    datetime.now(), "%Y_%m_%d_%H_%M_%S"
)  # 格式化當前時間
file_name = f"{file_timestamp}_multi_point.csv"  # 創建文件名
file_path = f"./data/{file_name}"  # 設置文件路徑
# 創建並初始化 CSV 文件
...
class Cal_Rudder_Engine:
    ...
    def RudderAngleCalculation(ship_data, goal_point, yaw_rate):
        ...
        # K = K * 0.4  # 將 K 乘以一個小於 1 的係數來降低控制增益
        if distance >= 350:
            K = K * (1 - (distance / 855))  # 將 K 乘以一個小於 1 的係數來降低控制增益
        elif 350 > distance >= 250:
            K = K * (1 - (distance / 800))  # 將 K 乘以一個小於 1 的係數來降低控制增益
        elif 250 > distance >= 200:
            K = K * (1 - (distance / 750))  # 將 K 乘以一個小於 1 的係數來降低控制增益
        elif 200 > distance > 0:
            K = K * 2.4  # 將 K 乘以一個小於 1 的係數來降低控制增益
        ...
        # 限制舵角輸出在 (-25, 25) 範圍內
        delta_value = delta[0, 0]  # 提取單一值
        delta_value_limited = max(
            min(delta_value, 25), -25
        )  # 將舵角限制在 -25 到 25 之間
        # delta_value_limited = delta_value_limited * (-1)  # 根據需求反向舵角
        delta_value_limited = int(delta_value_limited)
...
def DataTransmitter(command=None, command_data=None):
    command_dict = {
        "circle_right": {"Command": 8, "Range": 0.01, "Speed": 0.6},
        "circle_left": {"Command": -8, "Range": 0.01, "Speed": 0.6},
        "stop": {
            "Command": 666,
            "Left_Speed": 0,
            "Right_Speed": 0,
    ...
    try:
        ...
        if response.status_code == 200:
            # print(f"指令成功發送: {command_data}\n")
            print("=====================!next!=====================\n")
            return True
        else:
            print(f"指令發送失敗: {response.status_code}，重試")
    except requests.RequestException as e:
        print(f"API 請求失敗: {e}")


# def connect_control():
#     command_data = {'Command': 900}
#     DataTransmitter(command_data=command_data)
#     time.sleep(1)
#     command_data = {'Command': 901}
#     DataTransmitter(command_data=command_data)
# 主程式入口
def main_multi(goal_list, loop=False):
    ...
    # connect_control()
    # time.sleep(3)
    time_series = [None, None]  # 單位：秒
    yaw_series = [None, None]  # 單位：弧度
    yaw_rate = 0
    pre_rudder = None
    new_rudder = 0
    rudder_count = 0
    final_point = False
    # check = input('check:')
    if len(goal_list) > 1 and loop == True:
    ...
    for i in range(len(goal_list)):
        ...
        while True:
            ...
            if all(value is not None for value in ship_data.values()):
                ...
                current_yaw = ship_data["Yaw"]  # 當前yaw值

                # 更新time_series和yaw_series
                ...
                if time_series[0] is not None and yaw_series[0] is not None:
                    try:
                        ...
                        delta_t = np.diff(time_series)  # 計算每個相鄰時間點的時間差
                        # 確保 delta_t 不為零以避免除以零錯誤
                        if delta_t[0] != 0:
                            yaw_rate = delta_yaw[0] / delta_t[0]  # 取出第一個值作為標量
                            yaw_rate = float(yaw_rate)  # 確保是標量
                        else:
                            yaw_rate = 0

                    except Exception as e:
                        print(f"計算 yaw_rate 出現錯誤: {e}")
                        yaw_rate = 0
                else:
                    yaw_rate = 0
                distance = Cal_Rudder_Engine.calculate_distance(
                ...
                rudder_calc, target_lat, target_lon, yaw_error = (
                    Cal_Rudder_Engine.RudderAngleCalculation(
                        ship_data,
                        goal_point,
                        yaw_rate if isinstance(yaw_rate, np.ndarray) else 0,
                    )
                )

                new_rudder = rudder_calc
                if new_rudder == pre_rudder:
                    rudder_count += 1  # 如果與上一舵角相同，累加次數
                else:
                    rudder_count = 0  # 舵角有變則重置計數器

                # 根據累積的重複次數微調舵角，讓舵角持續漸變而非固定
                rudder_calc += np.sign(new_rudder) * min(rudder_count, 24)
                ...
                if (
                    ...
                    and target_lon == 0
                    and yaw_error == 0
                    and final_point == True
                ):
                    # DataCollector.DataCollector(threads_data['gps_port'], threads_data['gps_baudrate'],threads_data['ahrs_port'], threads_data['ahrs_baudrate'],switch='close')
                    DataTransmitter(command="stop")
                    # time.sleep(1)
                    break
                elif (
                    rudder_calc == 0
                ...
                with open(file_path, "a", newline="") as csvfile:
                    ...
                    csv_writer.writerow(
                        [
                            ...
                            ship_data["Heading"],  # Heading
                            ship_data["Lat"],  # Latitude
                            ship_data["Lon"],  # Longitude
                            ship_data["Roll"],  # Roll
                            ship_data["Pitch"],  # Pitch
                            ship_data["Yaw"],  # Yaw
                            target_lat,  # Target Latitude
                            target_lon,  # Target Longitude
                            yaw_error,  # Yaw Error
                            rudder_calc,  # Rudder_calc
                            distance,
                        ]
                    )
                now_gps = ship_data["Lat"], ship_data["Lon"]
                target_point = [None, None]
                target_point[0], target_point[1] = target_lat, target_lon
                ...
                print(f'Yaw is :{ship_data["Yaw"]}')
                print(f'Heading is :{ship_data["Heading"]}')
                print(
                    f"目前座標:{now_gps}\n目標點座標:{target_point}\n目標點距離:{distance} "
                )
...
main_multi(
    [
        [22.867241653727508, 120.20464501135687],
        [22.869184769825363, 120.20441978874305],
        [22.869213059307523, 120.20657986394784],
        [22.86715674695946, 120.206600313567],
    ],
    loop=True,
)  # 矩形
...
# main_multi(
#     [
#         [22.866148234576745, 120.20457934054345],
#         [22.867517038340054, 120.20659011963158],
#         [22.869148570360117, 120.20496949817199],
#         [22.871167248334977, 120.20664649465209],
#     ],
#     loop=False,
# )  # S型
