...
ouster_front_distance = None
ouster_event = threading.Event()


def collect_ouster_data():
    global ouster_front_distance
    while True:
        ouster_event.wait()
        try:
            distance = ouster_distance(sensor_ip="192.168.50.131", pos="front")
            ouster_front_distance = distance if distance is not None else None
...
class Cal_Rudder_Engine:
    ...
    def RudderAngleCalculation(ship_data, goal_point, yaw_rate):
        ...
        # # 加入滑動平均，平滑舵角輸出
        # rudder_history = Cal_Rudder_Engine.RudderAngleCalculation.rudder_history
        # rudder_history.append(delta_value)
        # if len(rudder_history) > 5:
        #     rudder_history.pop(0)
        # smoothed_delta = np.mean(rudder_history)
...
main_multi(
    [
        [22.864882384980746, 120.20118235645143],
        [22.865760249413437, 120.20073542570358],
    ],
    loop=False,
)  # 靠泊
...
# main_multi(
#     [
#         [22.86757395342671, 120.206102944323890],
#         [22.867379337411485, 120.20757054496363],
#     ],
#     loop=False,
# )  # 靠泊
